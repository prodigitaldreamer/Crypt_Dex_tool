...
class SymbolsStorage {
    _exchangesList = ['NYSE', 'FOREX', 'AMEX'];
    _symbolsInfo = {};
    _symbolsList = [];
    _datafeedUrl;
    _readyPromise;
    _datafeedSupportedResolutions;
    _requester;
    ...
    _init() {
        ...
        return Promise.all(promises)
            ...
            (0, helpers_1.logMessage)('SymbolsStorage: All exchanges data loaded');
        });
    }
    _requestExchangeData(exchange) {
        return new Promise((resolve, reject) => {
            this._requester.sendRequest(this._datafeedUrl, 'symbol_info', { group: exchange })
                ...
                .catch((reason) => {
                (0, helpers_1.logMessage)(`SymbolsStorage: Request data for exchange '${exchange}' failed, reason=${(0, helpers_1.getErrorMessage)(reason)}`);
                resolve();
            });
        });
    }
